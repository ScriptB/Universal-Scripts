if MenuNexus_LOADED and not _G.MenuNexus_DEBUG == true then
    -- error("Nexus is already running!", 0)
    return
end

pcall(function() getgenv().MenuNexus_LOADED = true end)
...
-- Asuneteric
local menuassets = {
    ["Nexus/assets/bindsandplugins.png"] = "rbxassetid://5147695474",
    ["Nexus/assets/close.png"] = "rbxassetid://5054663650",
    ["Nexus/assets/editaliases.png"] = "rbxassetid://5147488658",
    ["Nexus/assets/editkeybinds.png"] = "rbxassetid://129697930",
    ["Nexus/assets/edittheme.png"] = "rbxassetid://4911962991",
    ["Nexus/assets/editwaypoints.png"] = "rbxassetid://5147488592",
    ["Nexus/assets/imgstudiopluginlogo.png"] = "rbxassetid://4113050383",
    ["Nexus/assets/logo.png"] = "rbxassetid://128429351840104",
    ["Nexus/assets/minimize.png"] = "rbxassetid://2406617031",
    ["Nexus/assets/pin.png"] = "rbxassetid://6234691350",
    ["Nexus/assets/reference.png"] = "rbxassetid://3523243755",
    ["Nexus/assets/settings.png"] = "rbxassetid://1204397029"
}
...
if makefolder and isfolder and writefile and isfile then
    pcall(function() -- good executor trust
        ...
        for _, folder in {"Nexus", "Nexus/assets"} do
            if not isfolder(folder) then
                makefolder(folder)
            end
        end
        for path in menuassets do
            if not isfile(path) and path ~= "Nexus/assets/logo.png" then
                writefile(path, game:HttpGet((path:gsub("Nexus/", assets))))
            end
        end
        if IsOnMobile then writefile("Nexus/assets/.nomedia") end
    end)
end
...
local getcustomasset_vape = getsynasset or getcustomasset or function(location) return "rbxasset://"..location end
local queueonteleport = syn and syn.queue_on_teleport or queue_on_teleport or function() end

local function displayErrorPopup(text, func)
	warn("[Nexus] " .. tostring(text))
...
local function vapeGithubRequest(scripturl)
	...
	if not suc or not res or res == "" then
		warn("[NexusVape] Failed to fetch: " .. scripturl .. " — " .. tostring(res))
		return nil
	end
	return res
end
...
local function downloadVapeAsset(path)
	if not isfile(path) then
		...
		if suc and req then writefile(path, req) else return "" end
	end
	return getcustomasset_vape(path)
end
...
local libSource = vapeGithubRequest("NewGuiLibrary.lua")
if not libSource or libSource == "" then
	warn("[NexusVape] Could not load Vape GUI library. Aborting GUI init.")
	shared.VapeExecuted = nil
	return
end
local libLoader, libErr = loadstring(libSource)
if not libLoader then
	warn("[NexusVape] Failed to parse Vape GUI library: " .. tostring(libErr))
	shared.VapeExecuted = nil
	return
end
GuiLibrary = libLoader()
if not GuiLibrary then
	warn("[NexusVape] Vape GUI library returned nil.")
	shared.VapeExecuted = nil
	return
end
...
local saveSettingsLoop = coroutine.create(function()
	repeat GuiLibrary.SaveSettings() task.wait(10) until not vapeInjected or not GuiLibrary
end)

local GUI = GuiLibrary.CreateMainWindow()
local Combat   = GuiLibrary.CreateWindow({Name="Combat",   Icon="vape/assets/CombatIcon.png",   IconSize=15})
local Blatant  = GuiLibrary.CreateWindow({Name="Blatant",  Icon="vape/assets/BlatantIcon.png",  IconSize=16})
local Render   = GuiLibrary.CreateWindow({Name="Render",   Icon="vape/assets/RenderIcon.png",   IconSize=17})
local Utility  = GuiLibrary.CreateWindow({Name="Utility",  Icon="vape/assets/UtilityIcon.png",  IconSize=17})
local World    = GuiLibrary.CreateWindow({Name="World",    Icon="vape/assets/WorldIcon.png",    IconSize=16})
local Friends  = GuiLibrary.CreateWindow2({Name="Friends", Icon="vape/assets/FriendsIcon.png",  IconSize=17})
local Targets  = GuiLibrary.CreateWindow2({Name="Targets", Icon="vape/assets/FriendsIcon.png",  IconSize=17})
local Profiles = GuiLibrary.CreateWindow2({Name="Profiles",Icon="vape/assets/ProfilesIcon.png", IconSize=19})

GUI.CreateDivider()
GUI.CreateButton({Name="Combat",   Function=function(cb) Combat.SetVisible(cb)  end, Icon="vape/assets/CombatIcon.png",   IconSize=15})
GUI.CreateButton({Name="Blatant",  Function=function(cb) Blatant.SetVisible(cb) end, Icon="vape/assets/BlatantIcon.png",  IconSize=16})
GUI.CreateButton({Name="Render",   Function=function(cb) Render.SetVisible(cb)  end, Icon="vape/assets/RenderIcon.png",   IconSize=17})
GUI.CreateButton({Name="Utility",  Function=function(cb) Utility.SetVisible(cb) end, Icon="vape/assets/UtilityIcon.png",  IconSize=17})
GUI.CreateButton({Name="World",    Function=function(cb) World.SetVisible(cb)   end, Icon="vape/assets/WorldIcon.png",    IconSize=16})
GUI.CreateDivider("MISC")
GUI.CreateButton({Name="Friends",  Function=function(cb) Friends.SetVisible(cb)  end})
GUI.CreateButton({Name="Targets",  Function=function(cb) Targets.SetVisible(cb)  end})
GUI.CreateButton({Name="Profiles", Function=function(cb) Profiles.SetVisible(cb) end})
GUI.CreateDivider()

local GUIColorSlider = {RainbowValue=false}
GUIColorSlider = GUI.CreateColorSlider("GUI Theme", function(h,s,v) GuiLibrary.UpdateUI(h,s,v) end)

local GeneralSettings = GUI.CreateDivider2("General Settings")
local GUISettings     = GUI.CreateDivider2("GUI Settings")

GUISettings.CreateToggle({Name="Notifications",   Function=function(cb) GuiLibrary.Notifications=cb end, Default=true})
GUISettings.CreateToggle({Name="Toggle Alert",    Function=function(cb) GuiLibrary.ToggleNotifications=cb end, Default=true})
GUISettings.CreateToggle({Name="Show Tooltips",   Function=function(cb) GuiLibrary.ToggleTooltips=cb end, Default=true})
GUISettings.CreateToggle({Name="Blur Background", Function=function(cb)
	GuiLibrary.MainBlur.Size = cb and 25 or 0
	game:GetService("RunService"):SetRobloxGuiFocused(GuiLibrary.MainGui.ScaledGui.ClickGui.Visible and cb)
end, Default=true})

local GUIbind = GUI.CreateGUIBind()

-- Notify shim so existing Nexus notify() calls work via Vape notifications
local function notify(title, text, duration)
	pcall(function()
		GuiLibrary.CreateNotification(title, text, duration or 3)
	end)
end

-- Cmdbar shim (Nexus commands typed into Vape's text input)
local Cmdbar = Instance.new("TextBox")
Cmdbar.Name = "NexusCmdbar"
Cmdbar.Parent = GuiLibrary.MainGui
Cmdbar.Visible = false
Cmdbar.Text = ""
Cmdbar.PlaceholderText = "Command Bar"

-- ScaledHolder shim so Nexus UI references don't error
local ScaledHolder = GuiLibrary.MainGui
local Holder = GuiLibrary.MainGui
...
-- Compatibility stubs for Nexus internals that reference old UI objects
local Example = Instance.new("TextButton"); Example.Visible = false; Example.Parent = GuiLibrary.MainGui
local Settings = Instance.new("Frame"); Settings.Visible = false; Settings.Parent = GuiLibrary.MainGui
local SettingsHolder = Instance.new("ScrollingFrame"); SettingsHolder.Visible = false; SettingsHolder.Parent = Settings
local Cmdbar_2 = Instance.new("TextBox"); Cmdbar_2.Visible = false; Cmdbar_2.Parent = GuiLibrary.MainGui
local Cmdbar_3 = Instance.new("TextBox"); Cmdbar_3.Visible = false; Cmdbar_3.Parent = GuiLibrary.MainGui
local CMDsF = Instance.new("ScrollingFrame"); CMDsF.Visible = false; CMDsF.Parent = GuiLibrary.MainGui
local cmdListLayout = Instance.new("UIListLayout"); cmdListLayout.Parent = CMDsF
local Notification = Instance.new("Frame"); Notification.Visible = false; Notification.Parent = GuiLibrary.MainGui
local Title_2 = Instance.new("TextLabel"); Title_2.Parent = Notification
local Text_2 = Instance.new("TextLabel"); Text_2.Parent = Notification
local CloseButton = Instance.new("TextButton"); CloseButton.Parent = Notification
local PinButton = Instance.new("TextButton"); PinButton.Parent = Notification
local Tooltip = Instance.new("Frame"); Tooltip.Visible = false; Tooltip.Parent = GuiLibrary.MainGui
local Title_3 = Instance.new("TextLabel"); Title_3.Parent = Tooltip
local Description = Instance.new("TextLabel"); Description.Parent = Tooltip
local logs = Instance.new("Frame"); logs.Visible = false; logs.Parent = GuiLibrary.MainGui
local Credits = Instance.new("TextBox"); Credits.Visible = false; Credits.Parent = GuiLibrary.MainGui
Credits.Text = "Developed By Asuneteric"
local Scale = Instance.new("UIScale"); Scale.Parent = GuiLibrary.MainGui
local IntroBackground = Instance.new("Frame"); IntroBackground.Visible = false; IntroBackground.Parent = GuiLibrary.MainGui
local Logo = Instance.new("ImageLabel"); Logo.Visible = false; Logo.Parent = GuiLibrary.MainGui
local KeybindsFrame = Instance.new("Frame"); KeybindsFrame.Visible = false; KeybindsFrame.Parent = Settings
local PositionsFrame = Instance.new("Frame"); PositionsFrame.Visible = false; PositionsFrame.Parent = Settings
local AliasesFrame = Instance.new("Frame"); AliasesFrame.Visible = false; AliasesFrame.Parent = Settings
local PluginsFrame = Instance.new("Frame"); PluginsFrame.Visible = false; PluginsFrame.Parent = Settings
local Holder_2 = Instance.new("ScrollingFrame"); Holder_2.Visible = false; Holder_2.Parent = KeybindsFrame
local Holder_3 = Instance.new("ScrollingFrame"); Holder_3.Visible = false; Holder_3.Parent = AliasesFrame
local Holder_4 = Instance.new("ScrollingFrame"); Holder_4.Visible = false; Holder_4.Parent = PositionsFrame
local Holder_5 = Instance.new("ScrollingFrame"); Holder_5.Visible = false; Holder_5.Parent = PluginsFrame
local Example_2 = Instance.new("Frame"); Example_2.Visible = false; Example_2.Parent = KeybindsFrame
local Example_3 = Instance.new("Frame"); Example_3.Visible = false; Example_3.Parent = AliasesFrame
local Example_4 = Instance.new("Frame"); Example_4.Visible = false; Example_4.Parent = PositionsFrame
local Example_5 = Instance.new("Frame"); Example_5.Visible = false; Example_5.Parent = PluginsFrame
local ReferenceButton = Instance.new("ImageButton"); ReferenceButton.Visible = false; ReferenceButton.Parent = GuiLibrary.MainGui
local SettingsButton = Instance.new("ImageButton"); SettingsButton.Visible = false; SettingsButton.Parent = GuiLibrary.MainGui
local selectChat = Instance.new("TextButton"); selectChat.Visible = false; selectChat.Parent = GuiLibrary.MainGui
local selectJoin = Instance.new("TextButton"); selectJoin.Visible = false; selectJoin.Parent = GuiLibrary.MainGui
local chat = Instance.new("Frame"); chat.Visible = false; chat.Parent = GuiLibrary.MainGui
local join = Instance.new("Frame"); join.Visible = false; join.Parent = GuiLibrary.MainGui
local scroll_2 = Instance.new("ScrollingFrame"); scroll_2.Visible = false; scroll_2.Parent = chat
local scroll_3 = Instance.new("ScrollingFrame"); scroll_3.Visible = false; scroll_3.Parent = join
local listlayout = Instance.new("UIListLayout"); listlayout.Parent = scroll_3
local AliasHint = Instance.new("TextLabel"); AliasHint.Visible = false; AliasHint.Parent = AliasesFrame
local PluginsHint = Instance.new("TextLabel"); PluginsHint.Visible = false; PluginsHint.Parent = PluginsFrame
local PositionsHint = Instance.new("TextLabel"); PositionsHint.Visible = false; PositionsHint.Parent = PositionsFrame
local KeybindEditor = Instance.new("Frame"); KeybindEditor.Visible = false; KeybindEditor.Parent = GuiLibrary.MainGui
local PluginEditor = Instance.new("Frame"); PluginEditor.Visible = false; PluginEditor.Parent = GuiLibrary.MainGui
local TopartFrame = Instance.new("Frame"); TopartFrame = TopartFrame; TopartFrame.Visible = false; TopartFrame.Parent = GuiLibrary.MainGui
local shade1,shade2,shade3,text1,text2,scroll = {},{},{},{},{},{}

-- ViewportTextBox stub (Nexus uses this for cmdbar conversion)
local ViewportTextBox = {convert = function(obj) return {View = Instance.new("Frame")} end}

-- [< End Vape GUI — Nexus command logic follows >] --
...
eventEditor = (function()
	...
	local main = create({
		...
		{5,"ImageLabel",{BackgroundColor3=Color3.new(1,1,1),BackgroundTransparency=1,Image=getcustomasset("Nexus/assets/close.png"),Parent={4},Position=UDim2.new(0,5,0,5),Size=UDim2.new(0,10,0,10),ZIndex=10,}},
		...
		{53,"ImageLabel",{BackgroundColor3=Color3.new(1,1,1),BackgroundTransparency=1,Image=getcustomasset("Nexus/assets/settings.png"),Parent={52},Position=UDim2.new(0,2,0,2),Size=UDim2.new(0,16,0,16),ZIndex=10,}},
	})
...
reference = (function()
	local main = create({
		...
		{114,"TextLabel",{BackgroundColor3=Color3.new(1,1,1),BackgroundTransparency=1,Font=3,Name="Text",Parent={112},Position=UDim2.new(0,8,0,30),Size=UDim2.new(1,-8,0,32),Text="You can join the 833s Discord server to get support with Nexus, and read up on more documentation such as the Plugin API.",TextColor3=Color3.new(1,1,1),TextSize=14,TextWrapped=true,TextXAlignment=0,ZIndex=10,}},
	...
	inviteButton.MouseButton1Click:Connect(function()
		if everyClipboard then
			toClipboard("https://discord.gg/Sakhwt7ChV")
		...
		inviteButton.Text = "Copy Discord Invite Link (https://discord.gg/Sakhwt7ChV)"
	end)
	dragGUI(main)
	main.Parent = ScaledHolder
...
function saves()
    if writefileExploit() and readfileExploit() and jsonAttempts < 10 then
        local readSuccess, out = readfile("Nexus_FE.Nexus", true)
        if readSuccess then
            if out ~= nil and tostring(out):gsub("%s", "") ~= "" then
                local success, response = pcall(function()
                    ...
                    if vtype(json.keepNexus, "boolean") then KeepNexus = json.keepNexus else KeepNexus = true end
                ...
                if not success then
                    ...
                    writefile("Nexus_FE.Nexus", defaults, true)
                    wait()
                    saves()
                end
            else
                writefile("Nexus_FE.Nexus", defaults, true)
                wait()
                local dReadSuccess, dOut = readfile("Nexus_FE.Nexus", true)
...
function updatesaves()
	if nosaves == false and writefileExploit() then
		...
		writefileCooldown("Nexus_FE.Nexus", HttpService:JSONEncode(update))
	end
end
...
if type(PluginsTable) == "table" then
    for i = #PluginsTable, 1, -1 do
        if string.sub(PluginsTable[i], -6) ~= ".Nexus" then
...
CMDs = {}
CMDs[#CMDs + 1] = {NAME = 'discord / support / help', DESC = 'Invite to the 833s Discord — Nexus by Asuneteric.'}
...
Players.LocalPlayer.OnTeleport:Connect(function(State)
	if KeepInfYield and (not TeleportCheck) and queueteleport then
		TeleportCheck = true
		queueteleport("loadstring(game:HttpGet('https://raw.githubusercontent.com/ScriptB/Infinite-Yield/refs/heads/main/Infinite%20Yield_fixed2.lua'))()")
	end
end)
...
addcmd('discord', {'support', 'help'}, function(args, speaker)
	if everyClipboard then
		toClipboard('https://discord.gg/Sakhwt7ChV')
		notify('Discord Invite', 'Copied to clipboard!\ndiscord.gg/Sakhwt7ChV')
	else
		notify('Discord Invite', 'discord.gg/Sakhwt7ChV')
	end
	if httprequest then
		httprequest({
...
addcmd('serverinfo',{'info','sinfo'},function(args, speaker)
	...
	task.spawn(function()
		...
		ExitImage.Image = getcustomasset("Nexus/assets/close.png")
		ExitImage.ZIndex = 10
...
task.spawn(function()
	notify("Phantom Suite", "Join our Discord: discord.gg/Sakhwt7ChV")
	while true do
		wait(300)
		notify("Phantom Suite", "Join our Discord: discord.gg/Sakhwt7ChV")
	end
