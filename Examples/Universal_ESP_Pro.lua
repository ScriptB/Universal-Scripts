...
local function updateHealthESP(player, health)
	if not health or not player.Character or not shouldUpdate() then return end
	
	local humanoid = player.Character:FindFirstChildOfClass("Humanoid")
	local rootPart = player.Character:FindFirstChild("HumanoidRootPart")
	
	if not humanoid or not rootPart or humanoid.Health <= 0 then
		health.background.Visible = false
		health.health.Visible = false
		return
	end
	...
	local healthColor = ESPConfig.Health.Healthy
	if healthPercent < 0.5 then
		healthColor = ESPConfig.Health.Damaged
	elseif healthPercent < 0.75 then
		healthColor = Color3.fromRGB(255, 255, 0)
	end
	
	-- Auto-scale
	local width = ESPConfig.Health.Width
	local height = ESPConfig.Health.Height
	if ESPConfig.Health.AutoScale then
	...
	-- Update background
	health.background.Size = Vector2.new(width, height)
	health.background.Position = Vector2.new(barX, barY)
	health.background.Visible = true
	
	-- Update health bar
	health.health.Size = Vector2.new(width * healthPercent, height)
	health.health.Position = Vector2.new(barX, barY)
	health.health.Color = healthColor
	health.health.Visible = true
end
...
local function updateTracerESP(player, tracer)
	if not tracer or not player.Character or not shouldUpdate() then return end
	...
	if ESPConfig.Tracer.From == "Center" then
		origin = Vector2.new(Camera.ViewportSize.X / 2, Camera.ViewportSize.Y / 2)
	elseif ESPConfig.Tracer.From == "Top" then
	...
	if onScreen then
		...
		-- Off-screen tracer to edge
		local screenCenter = Vector2.new(Camera.ViewportSize.X / 2, Camera.ViewportSize.Y / 2)
		local direction = (position - screenCenter).Unit
		local edgePoint = screenCenter + direction * math.min(screenCenter.X, screenCenter.Y)
...
local function createArrowESP(player)
	...
	return createDrawing("Triangle", {
		...
		Filled = ESPConfig.Arrow.Filled,
		Transparency = 1
	})
end

local function updateArrowESP(player, arrow)
	if not arrow or not player.Character or not shouldUpdate() then return end
	...
	-- Apply rainbow effect
	local color = ESPConfig.Arrow.Color
	if ESPConfig.Arrow.Rainbow or ESPConfig.Effects.Rainbow then
		color = getRainbowColor(tick(), ESPConfig.Effects.RainbowSpeed)
	end
	...
	if not onScreen then
		-- Manual screen space projection for 100% accuracy
		local screenCenter = Vector2.new(Camera.ViewportSize.X / 2, Camera.ViewportSize.Y / 2)
		local fov = math.rad(Camera.FieldOfView)
		local aspectRatio = Camera.ViewportSize.X / Camera.ViewportSize.Y
		
		-- Transform to camera space
		local cameraSpace = Camera.CFrame:PointToObjectSpace(rootPart.Position)
		
		-- Project to screen space manually
		local screenX = screenCenter.X + (cameraSpace.X / -cameraSpace.Z) * (screenCenter.Y / math.tan(fov / 2))
		local screenY = screenCenter.Y - (cameraSpace.Y / -cameraSpace.Z) * (screenCenter.Y / math.tan(fov / 2))
		
		local projectedPos = Vector2.new(screenX, screenY)
		
		-- Calculate angle from screen center to player
		local angle = math.atan2(projectedPos.Y - screenCenter.Y, projectedPos.X - screenCenter.X)
		
		-- Position arrow at fixed distance from center
...
local function initializeESP()
	if not isDrawingAvailable() then
		warn("⚠️ Drawing API not available!")
		return
	end
...
print("🎯 Universal ESP Pro functions exported to getgenv().UniversalESP")
    ...
    if setclipboard then
        ...
        print("📋 Universal ESP Pro Enhanced copied to clipboard!")
    else
        warn("⚠️ setclipboard function not available!")
    end
    
    -- Also print the script for manual copying
    print("📄 Script content ready for manual copying:")
    print(scriptContent)
end

-- Export dev copier function
getgenv().devCopy = devCopy
print("🔧 Dev copier added! Use getgenv().devCopy() to copy the script")
